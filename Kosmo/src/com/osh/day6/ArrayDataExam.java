...
public class ArrayDataExam {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < (intArray.length-1); i++) {
			most = Integer.parseInt(intArray[i]) > Integer.parseInt(intArray[i+1]) ? Integer.parseInt(intArray[i]) : Integer.parseInt(intArray[i+1]);
		}
		...
		*/
		...
		while(true) {
			if(index == 0 || index % 2 == 0) {
				sum += Integer.parseInt(intArray[index]);
				count++;
			}
			index++;
		...
		for(int i = 0; i < intArray.length ; i++) {
			...
			if(intArray[i] == -1) {
				if(i < 3) {
					...
					System.out.println(intArray[i-3]);
